...
- `https://api.nbrb.by/exrates/rates/dynamics/{Cur_ID}`
...
Для исторической переоценки можно загрузить ряд НБРБ по `USD`, `EUR`, `RUB` начиная с `2024-01-01`. Команда сохраняет историю по датам и при повторном запуске обновляет те же даты и добавляет новые.
...
При необходимости можно задать свой диапазон и набор валют:

```powershell
.\.venv\Scripts\python.exe manage.py sync_nbrb_rates --start-date 2024-01-01 --end-date 2026-05-10 --currencies USD EUR RUB
```
